  ...
  return (
    <SafeAreaView style={[styles.screen, themed.screen]}>
      <View style={styles.header}>
        <Text style={[styles.eyebrow, themed.accentText]}>
          Bases disponibles
        </Text>
        <Text style={[styles.title, themed.title]}>Datos almacenados</Text>
      </View>
      <ScrollView
        ...
        <Section title="Tipos de sesión" count={sessionTypes.length}>
          {sessionTypes.map((item) => (
            <Row
              key={item.sync_uuid}
              title={item.name}
              detail={`${item.duration_hours || 1} h`}
            />
          ))}
        </Section>
        ...
        <Section title="Clientes" count={clients.length}>
          {clients.slice(0, 40).map((item) => (
            <Row
              key={item.sync_uuid}
              title={item.name}
              detail={item.phone || "Sin teléfono"}
            />
          ))}
          {clients.length > 40 ? (
            <Text style={styles.more}>
              Se muestran los primeros 40 clientes.
            </Text>
          ) : null}
        </Section>
      </ScrollView>
